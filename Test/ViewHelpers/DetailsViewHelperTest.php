...
class DetailsViewHelperTest extends Testcase
{
    public function testSuccessDisplayBiscuitDetails()
    {
        ...
        $expected .= '<div class="container-fluid mt-2 d-flex justify-content-around"><form action="hiddenVerify.php" method="POST">';
        $expected .= '<input type="hidden" name="dunk" value="Digestive"><button type="submit" class="btn btn-lg btn-success">Dunk</button>';
        $expected .= '</form><form action="hiddenVerify.php" method="POST"><input type="hidden" name="flunk" value="1">';
        $expected .= '<button type="submit" class="btn btn-lg btn-danger">Flunk</button></form></div>';
