...
class BiscuitsViewHelper
{
    ...
    public static function displayBiscuitDetails(Biscuits $biscuit): string
    {
        ...
        if ($biscuit->getName() !== '') {
            ...
            $result .= '<button type="submit" class="btn btn-lg btn-success">Dunk</button></form>';
            $result .= '<form action="hiddenVerify.php" method="POST"><input type="hidden" name="flunk" value="' . $biscuit->getId() . '">';
            $result .= '<button type="submit" class="btn btn-lg btn-danger">Flunk</button></form></div>';
            return $result;
        } else {
